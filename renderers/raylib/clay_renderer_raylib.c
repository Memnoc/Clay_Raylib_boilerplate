...
Raylib_MeasureText(Clay_String *text, Clay_TextElementConfig *config) {
  measureCalls++;
...
void Clay_Raylib_Render(Clay_RenderCommandArray renderCommands) {
  ...
  for (int j = 0; j < renderCommands.length; j++) {
    ...
    case CLAY_RENDER_COMMAND_TYPE_TEXT: {
      ...
      DrawTextEx(fontToUse, cloned, (Vector2){boundingBox.x, boundingBox.y},
                 (float)renderCommand->config.textElementConfig->fontSize,
                 (float)renderCommand->config.textElementConfig->letterSpacing,
    ...
    case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
      Texture2D imageTexture =
          *(Texture2D *)renderCommand->config.imageElementConfig->imageData;
      DrawTextureEx(imageTexture, (Vector2){boundingBox.x, boundingBox.y}, 0,
                    boundingBox.width / (float)imageTexture.width, WHITE);
      break;
    }
      ...
      if (config->cornerRadius.topLeft > 0) {
        float radius = (config->cornerRadius.topLeft * 2) /
                       (float)((boundingBox.width > boundingBox.height)
                                   ? boundingBox.height
                                   : boundingBox.width);
        DrawRectangleRounded((Rectangle){boundingBox.x, boundingBox.y,
    ...
    case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
      ...
      case CUSTOM_LAYOUT_ELEMENT_TYPE_3D_MODEL: {
        ...
        DrawModel(customElement->model.model, positionRay.position,
